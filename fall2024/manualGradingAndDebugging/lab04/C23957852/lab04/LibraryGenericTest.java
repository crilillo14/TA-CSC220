...
public class LibraryGenericTest {
  ...
  public static void main(String[] args) {
    ...
    ArrayList<LibraryBook<String>> sortedBooks = lib1.getInventoryList();
    if (sortedBooks == null || sortedBooks.size() != 3) {
    	System.err.println("TEST FAILED: getInventoryList - incorrect size");
    	
    } else {
    	for (int i = 1; i < sortedBooks.size(); i++) {
    		if (sortedBooks.get(i - 1).getIsbn() > sortedBooks.get(i).getIsbn()) {
    			System.err.println("TEST FAILED: getInventoryList - books not sorted by ISBN");
        		break;
    		}
