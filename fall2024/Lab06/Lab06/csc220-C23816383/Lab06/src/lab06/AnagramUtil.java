...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(String filename){
        ...
        SortedString[] words = readFile(absolutePath);
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
        ...
        if (stringList == null || stringList.length == 0) {
            return new String[0];  // Return an empty array for empty input
        }
        ...
        */
        for (i = 1; i < sorted.length; i++) {
            if (areAnagrams(sorted[i-1], sorted[i])) {
                length++;
            } else {
                if (length > maxLength) {
                    maxLength = length;
        ...
        */
        ...
        for (int j = 0; j < maxLength; j++)
            toReturn[j] = sorted[end - maxLength + 1 + j].getUnsorted();
