...
public class InsertionSort<E extends Comparable<E>> {
    ...
    public void fit(E[] array) {
        ...
        double timeInMicroseconds = elapsed / 1000.0;
        int n = array.length;
        c = timeInMicroseconds / O(n);
    }
    ...
    public E[] sort(E[] array) {
        ...
        for (int i = 1; i < sorted.length; i++) {
            E key = sorted[i];  // Select the element to be placed in the correct position.
            int j = i - 1;

            // Move elements of sorted[0..i-1], that are greater than key, to one position ahead
            while (j >= 0 && sorted[j].compareTo(key) > 0) {
                sorted[j + 1] = sorted[j];  // Shift element to the right
                j = j - 1;
            }
            sorted[j + 1] = key;  // Insert the key in its correct position
        }
