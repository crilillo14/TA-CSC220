...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
    	...
    	MergeSort<SortedString> stringListSorted = new MergeSort<SortedString>();	
    	
        /* sort the input string list */
    	stringListSorted.sort(stringList);
        ...
        */
        ...
        for (i = 0; i < stringList.length; i++) {	
			temp_string[i] = stringList[i].getSorted();
			System.out.print("** checked 32 to be deleted stringList ** " + temp_string[i] + " ");
		}
        *****/
        ...
        if (areAnagrams(stringList[i], stringList[i + 1])) {
        	length = length + 1;	
		} else if (stringList.length > maxLength) {      
		    i = i + 1; 
		    maxLength = maxLength + 1;
  
    	} else if (stringList.length > end) {
    		i = i + 1; 
    		end = end + 1;       
    	}
      ...
      //  System.out.println(" ");
      //  System.out.println("** checked areAnagrams to be deleted toReturn[j] ** ");
        for (int j = 0; j < maxLength; j++) {
            toReturn[j] = stringList[j+end-maxLength+1].getSorted();
