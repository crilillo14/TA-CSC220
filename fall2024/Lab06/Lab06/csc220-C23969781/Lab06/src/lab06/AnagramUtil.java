...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
    	...
    	MergeSort<SortedString> mergeSorterForSortedStrings = new MergeSort<>(); 	

    	

        /* sort the input string list */
    	//Arrays.toString(mergeSorterForSortedStrings.sort(stringList));
        stringList = mergeSorterForSortedStrings.sort(stringList);
...
//        	if(end == stringList.length) {
//        		break;
//        	}
            if (areAnagrams(stringList[i], stringList[i+1])) {
                length++; // Increment the length of the current anagram group
            } else {
                // If current group is larger than the known max group, update max length and position
                if (length > maxLength) {
                    maxLength = length;
                    end = i;
                }
        ...
        */
        ...
        if (length > maxLength) {
            ...
            i = end - length + 1; // Update start for the last group
        }// end of if statement
