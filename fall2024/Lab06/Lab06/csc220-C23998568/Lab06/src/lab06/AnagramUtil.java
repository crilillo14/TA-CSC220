...
public class AnagramUtil {
    ...
    public static boolean areAnagrams(SortedString str1, SortedString str2){
        // TODO: Lab Part 2
    	if (str1.getUnsorted().length() !=  str2.getUnsorted().length()) return false;
    	
    	if(str1.getSorted().compareTo(str2.getSorted()) == 0) return true;
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
    	...
    	//InsertionSort<SortedString> sorter = new InsertionSort<SortedString>();
    	MergeSort<SortedString> sorter = new MergeSort<SortedString>();
    	SortedString[] sorted = sorter.sort(stringList);
        ...
        int end = 0, length = 0, i = 0, maxLength = 0;
        ...
        */
        for(int j = 0; j < sorted.length; j++) {  
        	if(j != sorted.length - 1 && areAnagrams(sorted[j], sorted[j+1])) {
        		length++;
        	}else {
        		if(length > maxLength) {
        			end = i;
        ...
        */
        ...
        /* Prepare to return. The following is almost the answer except
           for one thing...
        */
        String[] noGood = new String[0];
     	if(maxLength < 1) return  noGood;

        String[] toReturn = new String[maxLength + 1];
      
     	
        for (int j = 0; j <= maxLength; j++) {
            toReturn[j] = sorted[end + j].getUnsorted().toString();
        }
