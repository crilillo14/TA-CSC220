...
public class InsertionSort<E extends Comparable<E>> {
    ...
    public void fit(E[] array) {
        long startTime = System.nanoTime() / 1000; // dividing by 1000 to convert to microseconds
        this.sort(array);
        long endTime = System.nanoTime() / 1000;
        long elapsedTime = endTime - startTime;
        
        this.c = (double) elapsedTime / O(array.length);
    }
    ...
    public E[] sort(E[] array) {
        ...
        for (int i=1; i < sorted.length; i++) {
        	...
        	E currentVal = sorted[i];
        	int j = i;
        	
        	while (j > 0 && sorted[j - 1].compareTo(currentVal) > 0) {
        		sorted[j] = sorted[j - 1];
        		j--;
        	}
        	sorted[j] = currentVal;
