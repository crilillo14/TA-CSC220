...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) {
    	...
    	char[] unsortedChars = new char[unsorted.replaceAll(" ", "").length()];
    	
    	int charIndex;
    	
    	for (charIndex = 0; charIndex < unsortedChars.length; ++charIndex) {
    		unsortedChars[charIndex] = this.unsorted.toLowerCase().charAt(charIndex);
    	}
    	
    	Arrays.sort(unsortedChars);
    	
    	this.sorted = new String(unsortedChars);
    ...
    public int compareTo(SortedString other) {
        ...
        return this.getSorted().compareToIgnoreCase(other.getSorted()); // placeholder
