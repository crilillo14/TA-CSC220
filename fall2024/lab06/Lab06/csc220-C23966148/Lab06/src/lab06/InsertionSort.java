...
public class InsertionSort<E extends Comparable<E>> {
    ...
    public void fit(E[] array) {
    	...
    	int n = array.length;
        double complexity = O(n); 

        this.c = (double) timeTaken / complexity;
    ...
    public double predict(int n) {
    	...
    	double predictedTime = this.c * complexity;
    	
    	
    	return predictedTime; // placeholder
    }
    ...
    public E[] sort(E[] array) {
        ...
        for(int i=1; i < sorted.length; i++) {
            ...
            if (index instanceof String) {
                ...
                String currentString = (String) index;
                
                String lowerCase = currentString.toLowerCase();
            
            while (j >= 0 && ((String) sorted[j]).toLowerCase().compareTo(lowerCase) > 0) {
        	...
        	// FILL IN
            	...
            	while (j >= 0 && sorted[j].compareTo(index) > 0) {
                    sorted[j + 1] = sorted[j];
                    j--;
                }
                sorted[j + 1] = index;
