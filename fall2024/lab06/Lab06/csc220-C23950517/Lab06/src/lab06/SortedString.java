...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) {
    	...
    	char[] letterArray = new char [unsorted.length()];
    	for(int i = 0; i < letterArray.length; i++) {
    		letterArray[i] = Character.toLowerCase(unsorted.charAt(i));
    	}
    	Arrays.sort(letterArray); 
    	sorted = new String(letterArray);
    	}
    ...
    public int compareTo(SortedString other) {
        ...
        return (int)(sorted.compareTo(other.getSorted())); 
    }
