...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
        ...
        SortedString[] sortedList = new InsertionSort<SortedString>().sort(stringList);
        ...
        */
        ...
        while (i < sortedList.length - 1) {
            if (areAnagrams(sortedList[i], sortedList[i + 1])) {
                // If consecutive strings are anagrams, increment the length of the current group
                length++;
            } 
            else {
                // If the current group is larger than the known max group, update maxLength and end
            	if (length > maxLength) {
            	    maxLength = length;
            	    end = i;
            	}
                // Reset length to 1 for the new group
                length = 1;
            }
            i++;
        }
