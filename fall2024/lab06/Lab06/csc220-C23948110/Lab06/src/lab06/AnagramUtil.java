...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
    	...
    	//InsertionSort<SortedString> anagramSorter = new InsertionSort<SortedString>();
    	MergeSort<SortedString> anagramSorter = new MergeSort<SortedString>();

        /* sort the input string list */
    	stringList = anagramSorter.sort(stringList);

        /* The case where stringList is of size 1 or less */
    	if (stringList.length <= 1) {
    		return new String[0];
    		//return sortedAnagrams; //what to return???
    	}
        ...
        */
        for (i = 0; i < stringList.length - 1; i++)  { //is the -1 necessary???
