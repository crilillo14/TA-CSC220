...
public class MergeSort<E extends Comparable<E>> {
    ...
    public void fit(E[] array) {
        ...
        // Calculate the elapsed time in microseconds (1 microsecond = 1000 nanoseconds)
        double elapsedTime = (endTime - startTime) / 1000.0;

        // Get the length of the array
        int n = array.length;

        // Fit the constant c based on the formula: c = elapsedTime / O(n log n)
    ...
    private void sort(E[] array1, E[] array2, int first, int last) {
        ...
        while (a <= middle && b <= last) {
        	if (array1[a].compareTo(array1[b]) <= 0) {
                array2[i] = array1[a];  // If array1[a] <= array1[b], copy array1[a]
                a++;                    // Move pointer a to the next element in the left subarray
            } else {
                array2[i] = array1[b];  // If array1[a] > array1[b], copy array1[b]
                b++;                    // Move pointer b to the next element in the right subarray
            }
            i++;  // Move to the next position in array2
        }
        while (a <= middle) {
            array2[i] = array1[a];
            a++;
        ...
        // If there are remaining elements in the right subarray, copy them to array2
        while (b <= last) {
            array2[i] = array1[b];
            b++;
            i++;
