...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) {
        ...
        char[] chars = unsorted.toLowerCase().toCharArray();  // Convert to lowercase and char array
        Arrays.sort(chars);  // Sort the characters
        this.sorted = new String(chars);  // Convert the sorted char array back to a string
    }
    ...
    public int compareTo(SortedString other) {
        return this.sorted.compareTo(other.sorted);  // Compare based on sorted version
    }
