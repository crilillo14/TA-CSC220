...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) {
    	...
    	if(this.unsorted==null) {
    		this.sorted=null;
    	}else if (this.unsorted!=null) {
    		char[] tosorted = unsorted.toLowerCase().toCharArray(); 
    	 	Arrays.sort(tosorted);
    	 	this.sorted = new String(tosorted);
    ...
    public int compareTo(SortedString other) {
        // TODO: Lab Part 1.2
    	if (other == null) { throw new NullPointerException("dne");}
    	
    	return sorted.toLowerCase().compareTo(other.sorted.toLowerCase());
