...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) {
    	...
    	char[] copy = unsorted.toLowerCase().toCharArray();
    	Arrays.sort(copy);
    	this.sorted = String.valueOf(copy);
    ...
    public int compareTo(SortedString other) {
        // TODO: Lab Part 1.2
    	for(int i = 0; i < other.sorted.length(); i++) {
    		char otherCompare = other.sorted.charAt(i);
    		char thisCompare = this.sorted.charAt(i);
    		
    		if(thisCompare > otherCompare) {
    			return 1;
    		} else if (thisCompare < otherCompare) {
    			return -1;
    		} 
