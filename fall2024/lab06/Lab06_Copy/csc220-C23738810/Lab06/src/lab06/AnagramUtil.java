...
public class AnagramUtil {
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
        ...
        stringList = sorter.sort(stringList);  // Sort the input string list

        // Handle the case where stringList is of size 1 or less
        ...
        */
        for (i = 0; i < stringList.length - 1; i++) {
            if (AnagramUtil.areAnagrams(stringList[i], stringList[i + 1])) {
                // If the current and next strings are anagrams, increment the length
                length++;
            } else {
                // If not, check if the current group is the largest and update variables
                if (length > maxLength) {
                    maxLength = length;
                    end = i;
                }
        ...
        // Perform one last check after the loop ends to handle the last group
        if (length > maxLength) {
            maxLength = length;
