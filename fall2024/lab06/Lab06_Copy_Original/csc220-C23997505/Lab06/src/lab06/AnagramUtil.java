...
public class AnagramUtil {
    ...
    public static boolean areAnagrams(SortedString str1, SortedString str2){
        // TODO: Lab Part 2
        return str1.getSorted().equalsIgnoreCase(str2.getSorted()); // placeholder
    }
    ...
    public static String[] getLargestAnagramGroup(SortedString[] stringList){
        ...
        MergeSort<SortedString> sorting = new MergeSort<>();
        sorting.sort(stringList);
        ...
        */
        for (i = 1; i < stringList.length; i++) {
            if (stringList[i].getSorted().equals(stringList[i - 1].getSorted())) {
                length++;  // Same anagram group
            } else {
                // Check if the current group is the largest so far
                if (length > maxLength) {
                    maxLength = length;
                    end = i - 1; // End index of the largest group
                }
                length = 1; // Reset for the next group
            }
        }
        ...
        */
        ...
        return maxLength > 1 ? toReturn : new String[0]; // return the right thing
    }
