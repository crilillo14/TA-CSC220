...
public class SortedString implements Comparable<SortedString> {
    ...
    public SortedString(String unsorted) 
    {
    	...
    	this.sorted = "";
    	char[] charArrayVariable= this.unsorted.toLowerCase().toCharArray();
    	Arrays.sort(charArrayVariable);
    	for (int i = 0; i<charArrayVariable.length; i++)
    	{
    		sorted += String.valueOf(charArrayVariable[i]);
    	}
    }
    ...
    {
        // TODO: Lab Part 1.2
    	int comparison = sorted.compareTo(other.getSorted());
        return comparison; 
    }
