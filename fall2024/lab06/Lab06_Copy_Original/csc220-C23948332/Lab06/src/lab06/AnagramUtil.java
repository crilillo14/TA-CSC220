        ...
        for (i = 1; i < stringList.length; i++) {
           // if (stringList[i].equals(stringList[i + 1])) {
        	if(areAnagrams(stringList[i], stringList[i-1])) {
                length++; // Increase the group length if adjacent words are anagrams
            } else {
                // If the current group ends, check if it's the largest found so far
                if (length > maxLength) {
                    maxLength = length;
                    end = i-1;
                }
                length = 1; // Reset the length for the next group
            }
        }
