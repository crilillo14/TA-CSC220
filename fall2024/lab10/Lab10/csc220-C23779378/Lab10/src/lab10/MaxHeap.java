...
public class MaxHeap {
    ...
    public void offer(int element) {
        ...
        if (size < theData.length) { 
            ...
            while (cPos > 0 && theData[pPos] < theData[cPos]) { 
                swap(cPos, pPos);
                cPos = pPos;
                pPos = parent(cPos);
            }
    ...
    protected void siftDown(int index) {
		...
		if (l<size() && this.theData[l] > this.theData[par]) {
			par = l;
		}
		if (r<size() && this.theData[r] > this.theData[par]) {
			par = r;
		}
		l = this.leftChild(par);
		r = this.rightChild(par);
