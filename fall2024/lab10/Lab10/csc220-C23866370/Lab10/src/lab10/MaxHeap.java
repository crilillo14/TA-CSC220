...
public class MaxHeap {
     ...
     * and initialize all of its elements to be zero. 
     * 
    ...
    public MaxHeap(int[] arr) {
        ...
        heapify(size - 1);  // Build a valid max heap from the input array
    }
    ...
    public void offer(int element) {
        if (size >= theData.length) return;  // Ensure we don’t exceed array bounds
        ...
        // Adjust the element up the tree to maintain max-heap property
     ...
     */
    public int poll() {
        if (size == 0) return -1;  // Return -1 if the heap is empty

        int maxElement = theData[0];
        theData[0] = theData[size - 1];
        size--;

        // Restore max-heap property by sifting down the new root
        siftDown(0);
    ...
    protected void siftDown(int index) {
        while (!isLeaf(index)) {
            ...
            // If the current element is already larger than the larger child, we're done
            if (theData[index] >= theData[largerChild]) break;

            // Swap the current element with the larger child and continue
