...
public class MaxHeap 
{
    ...
    public MaxHeap(int[] arr)
    {
    	...
    	//System.out.println("Initial array: " + java.util.Arrays.toString(arr)); // Debugging statement
    	//heapify(size-1); // Heapifying the entire array 
    	//System.out.println("Heapified array: " + java.util.Arrays.toString(theData)); // Debugging statement
    	//for each element in arr (theData), put it in the correct spot using offer
    	for(int i : arr)
    	{
    		offer(i);
    	}
    ...
    public int poll() 
    {
    	...
    	if (size == 0)
    	{
    		return -1; // Indication for empty heap
    	}

        int maximum = theData[0];
        theData[0] = theData[size - 1];
        theData[size-1] = 0; // Setting the removed element's spot to 0
        size--;
    ...
    public void sort(int[] arr) 
    {
        ...
        //System.out.println("Sorting array: " + java.util.Arrays.toString(arr)); // Debugging statement
        heapify(size-1);
        //System.out.println("Array after heapify in sort: " + java.util.Arrays.toString(theData)); // Debugging statement
        
        while(size > 1)
        {
        	swap(0, size-1);
        	//System.out.println("Swapped max to end: " + java.util.Arrays.toString(theData)); // Debugging statement
        	size--;
        	siftDown(0);
        	//System.out.println("Array after siftDown in sort: " + java.util.Arrays.toString(theData)); // Debugging statement
    ...
    protected void heapify(int index) 
    {
        ...
        {
            siftDown(i);
            //System.out.println("Heapifying at index " + i + ": " + java.util.Arrays.toString(theData)); // Debugging Statement
        }
    ...
    {
        // TODO Assignment Part 1
    	//System.out.println("Sifting down index: " + index + " Current heap: " + Arrays.toString(Arrays.copyOf(theData, size))); // Debugging statement
    	int largest = index;
    	while (!isLeaf(largest)) 
    	{
            int leftChild = leftChild(largest);
            int rightChild = rightChild(largest);
            ...
            // If largest is still the current index, then the heap property is satisfied
            if (largest == index) 
            {
                break;
            }
            ...
            swap(index, largest);
            //System.out.println("Swapped " + index + " and " + largest + ": " + java.util.Arrays.toString(theData)); // Debugging statement
            // Move down to the largest child’s index and continue sifting down 
            index = largest;
        }
    }
}
