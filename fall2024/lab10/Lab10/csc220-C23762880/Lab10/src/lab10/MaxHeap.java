...
public class MaxHeap {
    ...
    public void offer(int element) {
    	if (size >= theData.length) {
            throw new IllegalStateException("Heap is full, cannot add element.");
        }

        theData[size] = element; // Insert the new element at the end of the heap
        int current = size; // Start from the last element
        ...
        while (current > 0 && theData[current] > theData[parent(current)]) {
            swap(current, parent(current)); // Swap with parent if the current is greater
            current = parent(current); // Move to the parent's position
        }
    }
    ...
    public int poll() {
        ...
        theData[size - 1] = 0; // Optional: Clear the last element (for better visualization)
        size--;


        siftDown(0); // Use siftDown to reorder
    ...
    protected void siftDown(int index) {
        ...
        if (largest != index) {  // Only swap if necessary
            swap(index, largest);
            siftDown(largest);  
        }
    }
