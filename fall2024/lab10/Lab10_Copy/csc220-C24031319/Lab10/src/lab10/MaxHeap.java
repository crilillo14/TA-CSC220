...
//    	}
    ...
    public int poll() {
    	...
    	int index = 0;
    	while (!isLeaf(index)) {
    		if (rightChild(index) < size) { //if has a right child 
    			if(theData[rightChild(index)] <= theData[leftChild(index)] ) { //left child is bigger than right
    				if (theData[leftChild(index)] < theData[index]) {
    					return max;
    				}
    				swap(leftChild(index), index);
    				index = leftChild(index);
    			}else { //right child is > left
    ...
    protected void siftDown(int index) {
    	while (!isLeaf(index)) {
            ...
            if (rightChildIndex < size && theData[rightChildIndex] > theData[leftChildIndex]) {
                largerChildIndex = rightChildIndex;
            }

            // If the current node is greater than or equal to the larger child, stop sifting down
            if (theData[index] >= theData[largerChildIndex]) {
                return;
            }

            // Swap with the larger child and update index to continue sifting down
