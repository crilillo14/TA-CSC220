...
public class MaxHeap {
    ...
    public MaxHeap(int maxsize) {
    	...
    	// initialize theData to a new array of integers filled with 0's
    ...
    public MaxHeap(int[] arr){
    	...
    	// for each element in arr, put in theData in correct spot using offer
    	for(int i:arr)
    		offer(i);
    	
    	// set the size to be the length
    	size = arr.length;  
    }
    ...
    public void offer(int element) {
    	...
    	// the data goes through the elements to find the current value
    	theData[size] = element;
        int current = size;
        // increase size by one when adding an element
        size++;
        // swap with parent until in correct spot
    ...
    public int poll() {
    	...
    	// Set the last position to 0, so it is no longer part of the heap
        int max = theData[0];
        theData[0] = theData[size - 1];
        theData[size - 1] = 0; 
        
        // decrease size after updating
        size--;
        siftDown(0);
    ...
    public void sort(int[] arr) {
        ...
        // use helper methods heapify and siftDown to loop through and sort the Heap
        heapify(size - 1);
        while (size > 1) {
            swap(0, size - 1);
            size--;
    ...
    protected void heapify(int index) {
        // if the heap is empty, retutn
    	if (index == 0) {
            return;
        }
    		// loop through and sift down for each element 
