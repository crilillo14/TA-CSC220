...
public class MaxHeap {
    ...
    public void sort(int[] arr) {
        ...
        for (int i = size - 1; i > 0; i--) {
            swap(0, i);  // Move current root (max element) to the end of the array
            size--;      // Decrease the size of the heap
            siftDown(0); // Restore the heap property after swapping
        }

        // Restore the original size of the heap after sorting is complete.
        this.size = arr.length;
    }
