...
public class MaxHeap {
    ...
    public void offer(int element) {
        // TODO for Lab Part 3
    	int spot = size();
    	theData[spot] = element;
    	while(spot!= 0 && element>= theData[parent(spot)]) {
    		if(element> theData[parent(spot)]) {
    ...
    public int poll() {
    	...
    	int value = theData[size()-1]; 
    	theData[size()-1]= 0;
    	int spot =0;
    	while(spot!= size() && value<= theData[leftChild(spot)] || value <= theData[rightChild(spot)]) {
    		if(value< theData[leftChild(spot)]) {
    			swap(spot,leftChild(spot));
    			spot = leftChild(spot);
    		}else if(value< theData[rightChild(spot)]) {
    			swap(spot,rightChild(spot));
    			spot = rightChild(spot);
    		}
    	}
    	size--;
        return save; // Placeholder
    }
