...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr){
        ...
        // Call sort to build a max heap and sort the array
    ...
    public void offer(int element) {
    	...
    	size++;
    	while(parentIndex >= 0 && theData[parentIndex] < theData[index] ) {
    	...
    	if(parent(size) >= 0 && theData[parent(size)] < theData[size]) {
    		swap(theData[parent(size)], size);
    		}
    	*/
    ...
    public int poll() {
    	...
    	swap(0, size -1);
    	size--;
    	int result = theData[size];
    	theData[size] = 0;
    	int index = 0;
    	int rightIndex = rightChild(index);
    	int leftIndex = leftChild(index);
    	while( (leftIndex < size || rightIndex < size) && 
    			(theData[index] < theData[leftIndex] || theData[index] < theData[rightIndex])){
    		
    		if(rightIndex >= size) {
    			
    			if(theData[index] < theData[leftIndex]) {
    		...
    		}//the case if there is no right index
    		else {
    			if(theData[rightIndex] >= theData[leftIndex]) {
