...
public class MaxHeap {
    ...
    public void offer(int element) {
    	theData[this.size++] = element;
    	int i = size - 1;
    	
    	while (i != 0 && theData[i] > theData[this.parent(i)]) {
    ...
    public int poll() {
        ...
        while (!this.isLeaf(i) && !done) {
        	// left child must exist if it is not a leaf :)
        	int left = theData[this.leftChild(i)];
        	
        	int right = Integer.MIN_VALUE;
        	if (this.rightChild(i) < this.size) {
        		right = theData[this.rightChild(i)];
        	}
        	
        	int maxIndex;
        	if (left > right) {
        		maxIndex = this.leftChild(i);
        	} else {
        		maxIndex = this.rightChild(i);
        	}
        	
        	if (theData[i] > theData[maxIndex]) {
        		done = true;
        	} else {
