...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	while (theData[i] > theData[parent]) {
    ...
    public int poll() {
    	...
    	while (theData[i] < Math.max(theData[left], theData[right])) {
    		
    		if (theData[i] < theData[left]) {
    ...
    protected void siftDown(int index) {
    	...
    	if (left < size && theData[max] < theData[left]) {
    		
    		max = left;
    	}
    	
    	if (right < size && theData[max] < theData[right]) {
