...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	if (size < theData.length) {
    		...
    		size++;
    		while (parent(index) >= 0 && theData[index] > theData[parent(index)]) {
    ...
    public int poll() {
        // TODO for Lab Part 4
    	if (size > 0) {
    		int maxElement = theData[0];
    		theData[0] = theData[size - 1];
    		theData[size - 1] = 0;
    		size--;
    		int index = 0;
    		while ((leftChild(index) < size && theData[index] < theData[leftChild(index)]) || (rightChild(index) < size && theData[index] < theData[leftChild(index)])) {
    			//System.out.println("index: " + index + "\t left child index: " + leftChild(index) + "\t right child index: " + rightChild(index));
...
//        System.out.println();
    ...
    protected void heapify(int index) {
        ...
        for (int i = parent(index); i >= 0; i--) {
        	//System.out.println("Heap sift: " + i);
            siftDown(i);
        }
    }
    ...
    protected void siftDown(int index) {
        ...
        // TODO Assignment Part 1
//    	if ((leftChild(index) < size && theData[index] < theData[leftChild(index)]) || (rightChild(index) < size && theData[index] < theData[leftChild(index)])) {
//			if (rightChild(index) < size && theData[leftChild(index)] < theData[rightChild(index)]) {
//				swap(index, rightChild(index));
//				//index = rightChild(index);
...
//    		return;
//    	}
//    	if(theData[leftChild(index)] > theData[index] || (rightChild(index) < size && theData[rightChild(index)] > theData[index])) {
//    		if (rightChild(index) < size && theData[leftChild(index)] < theData[rightChild(index)]) {
//				swap(index, rightChild(index));
//				siftDown(rightChild(index));
//			} else {
...
//    	}
    	...
    	if(left < size && theData[left] > theData[max]) {
    		max = left;
    	}
    	
    	if (right < size && theData[right] > theData[max]) {
    		max = right;
    	}
    	
    	if (max != index) {
    		//System.out.println("swap " + theData[index] + " " + theData[max]);
    		swap(index, max);
    		siftDown(max);
    	}
