...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr){
    	...
    	this.theData = arr.clone();  // Copy the input array
        this.size = arr.length;      // Set the size

        // Heapify the array to turn it into a max-heap
       
        for (int i = parent(size - 1); i >= 0; i--) {
            siftDown(i); // Restore the max-heap property
        }
    ...
    protected void siftDown(int index) {
        ...
        if (leftChildIndex < size && theData[leftChildIndex] > theData[largestIndex]) {
            largestIndex = leftChildIndex;
        }
        if (rightChildIndex < size && theData[rightChildIndex] > theData[largestIndex]) {
