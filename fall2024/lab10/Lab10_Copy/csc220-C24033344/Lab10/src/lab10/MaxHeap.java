...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	theData[elementIndex] = element;
    	while (theData[parent(elementIndex)] < theData[elementIndex]) {
    		swap(parent(elementIndex), elementIndex);
    ...
    public int poll() {
        // TODO for Lab Part 4
    	if (size != 0) {
    		int elementReturned = theData[0];
    		theData[0] = theData[size - 1];
    		theData[size-- - 1] = 0;
    		int removedIndex = 0;
    		while (true) {
    			if (theData[leftChild(removedIndex)] >= theData[removedIndex]) {
    				swap(leftChild(removedIndex), removedIndex);
    				removedIndex = leftChild(removedIndex);
    			}
    			else if (theData[rightChild(removedIndex)] >= theData[removedIndex]) {
    				swap(rightChild(removedIndex), removedIndex);
    				removedIndex = rightChild(removedIndex);
    			}
    			else {
    				break;
    			}
    ...
    protected void siftDown(int index) {
    	...
    	if (leftChild(index) < size && theData[leftChild(index)] >= theData[index]) {
    	...
    	if (rightChild(index) < size && theData[rightChild(index)] >= theData[index]) {
    			
    		if (leftChild(index) < size && theData[leftChild(index)] > theData[rightChild(index)]) {
    			swap(leftChild(index), index);
    			siftDown(leftChild(index));
    		}
    		else {
