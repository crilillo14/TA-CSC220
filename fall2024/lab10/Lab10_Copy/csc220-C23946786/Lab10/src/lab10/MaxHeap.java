...
public class MaxHeap {
	...
	/**
	 * Constructor that initializes a heap and organizes the input array to form a
	 * valid max heap.
	 * 
	 ...
	 * 
	 * @return a comma-separated string of the heap elements NOTE this method should
	 *         *not* be modified.
	 */
	...
	/**
	 * Prints the tree structure of the heap with indentation indicating depth. NOTE
	 ...
	 * @return true if the heap data matches the input array, false otherwise NOTE
	 *         this method should not be modified.
	 */
	 ...
	 * 
	 * Adds element to the latest available position in the heap array, then adjusts
	 * the heap to maintain the max heap property
	 * 
	 * @param element the element to be added to the heap pre: theData is in heap
	 *                order post: the element is added and theData is in heap order
	 */
	 ...
	 * 
	 * To maintain the Heap Property: Swap the root with the last leaf, then sift
	 * the new root down to restore heap property
	 * 
	 * If the heap is empty, returns -1.
	 * 
	 * @return the maximum element in the heap, or -1 if empty pre: theData is in
	 *         heap order post: the max item is removed, and theData is in heap
	 *         order
	 */
	public int poll() {
		// TODO for Lab Part 4
	 ...
	 * 
	 * @param arr the array to be sorted pre: the array is unordered post: the array
	 *            is sorted in ascending order NOTE do *not* modify the signatures
	 *            of sort(), heapify(), or siftDown()
	 */
	...
	/**
	 * Helper method to reorganize the array into a valid max heap structure. Starts
	 * at the last parent node and calls siftDown() on each parent node until the
	 * root.
	 * 
	 ...
	 * parent, the largest child value is swapped with the parent, and the node is
	 * sifted down until no sifting is needed or there are no more children.
	 * 
	...
	protected void siftDown(int index) {
		...
		while (leftChild(index) < size) {
			int swapIndex = leftChild(index);
				...
				if(rightChild(index) < size && theData[rightChild(index)] > theData[leftChild(index)]) {
					swapIndex = rightChild(index);
				}
				
				if(theData[index] >= theData[swapIndex]) {
					return;
				}
	...
	protected void siftUp(int index) {
		while (index > 0) {
			if (theData[parent(index)] < theData[index]) {
