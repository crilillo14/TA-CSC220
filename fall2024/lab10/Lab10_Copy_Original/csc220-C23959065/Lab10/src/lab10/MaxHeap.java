...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	size++;
    	while(currentIndex > 0 && theData[parentIndex] < theData[currentIndex]){
    ...
    public int poll() {
    	...
    	swap(0,size-1);
    	size--;
    	int max = theData[size];
    	int currentIndex = 0;
    	
    	while(leftChild(currentIndex) < size) {
    			if(leftChild(currentIndex) > size && rightChild(currentIndex) > size ) {
    				int temp = theData[size];
    				theData[size] = 0;
    				
    				return temp;
    			}
    			
    			if(theData[currentIndex] < theData[leftChild(currentIndex)] && size >= leftChild(currentIndex)) {
    				swap(currentIndex,leftChild(currentIndex));
    				currentIndex = leftChild(currentIndex);
    			}else if(theData[rightChild(currentIndex)] > theData[currentIndex] && rightChild(currentIndex) < size) {
    ...
    private void siftDown(int index) {
        // TODO Assignment Part 1
            ...
            if ((theData[index] < theData[left]) || (theData[index] < theData[right])) {
            	if(theData[left] > theData[right]) {
            		swap(index,left);
            		siftDown(left);
            	}else if((theData[left] <= theData[right])){
            		swap(index,right);
            		siftDown(right);
            	}
