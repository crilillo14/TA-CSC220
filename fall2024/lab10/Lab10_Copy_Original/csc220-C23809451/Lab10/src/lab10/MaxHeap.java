...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr) {
        // Initialize the heap and set the size to the array length
    ...
    public void offer(int element) {
        if (size < theData.length) {
            ...
            int current = size;
            while (theData[current] > theData[parent(current)] && current > 0) {
    ...
    private void siftDown(int index) {
        ...
        // If largest is not the parent, swap and continue sifting down
        if (largest != index) {
            swap(index, largest);
            siftDown(largest);
        }
