...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr) {
        this.theData = arr;  // Initialize the heap data to the input array
        this.size = arr.length;  // Set the heap size to the array length
        heapify(size - 1);  // Heapify the array to satisfy max-heap properties
    }
    ...
    public int poll() {
        ...
        int maxItem = theData[0]; // Store the maximum item (root)
        theData[0] = theData[size - 1]; // Move the last leaf to the root
        size--; // Reduce the size of the heap

        // Optionally, set the last position to zero to avoid duplicates
        theData[size] = 0; // Set the last index to zero (optional, to clean up)

        int current = 0; // Start sifting down from the root
        ...
        while (true) {
            ...
            // If the largest is still the current, break
            if (largest == current) {
                break;
            }
        ...
        return maxItem; // Return the maximum item that was removed
    }
    ...
    private void siftDown(int index) {
        ...
        // If the largest value is not the current index, swap and continue sifting down
        if (largest != index) {
            swap(index, largest);
