...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr){
    	...
    	for (int i = (theData.length / 2) - 1; i >= 0; i--) {
            siftDown(i);
        }
    ...
    public void offer(int element) {
    	...
    	while(theData[parentPosition] < element){
    ...
    public int poll() {
    	...
    	while(theData[leftChildPosition] > theData[currentPosition] || theData[rightChildPosition] > theData[currentPosition]) {
    ...
    private void siftDown(int index) {
        ...
        while (leftChild(currentPosition) < size) {  // Loop while there is at least one child
            int leftChildPosition = leftChild(currentPosition);
            int rightChildPosition = rightChild(currentPosition);
            int largerChildPosition = leftChildPosition;  // Assume left is larger initially

            // Determine if the right child is larger
            if (rightChildPosition < size && theData[rightChildPosition] > theData[leftChildPosition]) {
                largerChildPosition = rightChildPosition;
            }

            // If the current node is larger or equal, the heap property is satisfied
            if (theData[currentPosition] >= theData[largerChildPosition]) {
                break;
            }

            // Swap current node with the larger child
            swap(currentPosition, largerChildPosition);

            // Move down to the position of the swapped child
            currentPosition = largerChildPosition;
        }
    }
