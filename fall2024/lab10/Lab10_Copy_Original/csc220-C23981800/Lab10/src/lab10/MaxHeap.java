...
public class MaxHeap {
    ...
    public void offer(int element) {
        if (size >= theData.length) {
            return; // Normally, you would resize the array, but instructions say to ignore overflow.
        }
        theData[size] = element;
        int current = size;
        size++;
        
        // Sift up to restore the heap property
    ...
    public int poll() {
        ...
        int maxItem = theData[0];  // Store the root element to return later.
        theData[0] = theData[size - 1];  // Move the last item to the root position.
        size--;  // Reduce the heap size as the last element is now removed.
        siftDown(0);  // Restore the heap property starting from the root.
        return maxItem;  // Return the previously stored root element.
    }
    ...
    private void siftDown(int index) {
        ...
        int left = leftChild(index);  // Calculate the left child index.
        int right = rightChild(index);  // Calculate the right child index.

        // If the left child exists and is greater than the current node, mark it as largest.
        ...
        // If the largest isn't the current index, swap and continue sifting down.
        if (largest != index) {
            swap(index, largest);
            siftDown(largest);  // Recursively adjust the sub-tree to ensure the heap property.
        }
