...
public class MaxHeap 
{
     ...
     * post: the element is added and theData is in heap order
     ...
     While (current node’s value is not greater than its parent){  swap current node’s value with parent’s value
     current node = parent node
     }
     */
    //(keep this in mind, when you are testing your code - not to add more than the array can contain)
    ...
    public int poll() 
    {
        ...
        int maximumElement = theData[0];
        
        size--;
        theData[0] = theData[size]; 
        siftDown(0);
        theData[size] = 0;
        return maximumElement; // Placeholder
    }
