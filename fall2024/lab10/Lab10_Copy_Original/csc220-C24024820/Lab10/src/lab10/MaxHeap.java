...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	while(ind>0) {
    		if(theData[ind] > theData[parent(ind)]) {
    			swap(ind,parent(ind));
    		}
    		ind=parent(ind);
    	}
    ...
    public int poll() {
    	...
    	while(!(leftChild(ind) > size-1)) {
    		//while there is a leftchild or rightchild
    		if((theData[leftChild(ind)] > theData[ind]) || (theData[rightChild(ind)] > theData[ind])) {
        		//if need to swap
        		if(!(rightChild(ind) >= size)) {
            		//if there's 2 children
            		if(theData[leftChild(ind)] > theData[rightChild(ind)]) {
            			//leftchild larger
    ...
    private void siftDown(int index) {
    	...
    	while(!(leftChild(ind) > size-1)) {
    			...
    			if(!(rightChild(ind) >= size)) {
    	    		...
    	    		if(theData[leftChild(ind)] > theData[ind] && theData[leftChild(ind)] > theData[rightChild(ind)]) {
    	    			//if leftchild larger than parent and rightchild, swap with leftchild
    	    			swap(ind,leftChild(ind));
    	    			ind=leftChild(ind);
    	    		}
    	    		else if (theData[rightChild(ind)] > theData[ind] && theData[rightChild(ind)] >= theData[leftChild(ind)]) {
    	        		//if rightchild larger than parent and leftchild, swap with rightchild
    	        		swap(ind,rightChild(ind));
    	        		ind=rightChild(ind);
    	        	}
    	    		else {
    	    		...
    	    		//there's only a leftchild
    	    		if(theData[leftChild(ind)] > theData[ind]) {
    	        		//if leftchild larger than parent, swap
    	        		swap(ind,leftChild(ind));
    	        		ind=leftChild(ind);
    	    		}
    	    		else {
