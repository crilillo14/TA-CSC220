...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	theData[i] = element;
    	size++;
    	while (0 <= (i-1)/2 && (i-1)/2 < size){
    		
    		if (theData[(i-1)/2] < theData[i]) {
    			
    			
    			int temp = theData[(i-1)/2];
    			theData[(i-1)/2] = theData[i];
    			theData[i] = temp;
     ...
     * 
     * if i have at least one child and one child is bigger than me
     * 	then, swap with the larger of my children (if I have two children)
     * 
    ...
    public int poll() {
    	...
    	if (size == 0) {
    		...
    		while (2*i + 2 < size) {
    				...
    				if (theData[(2*i + 2)] > theData[i] || theData[(2*i + 1)] > theData[i]) {
    					
    					if (theData[(2*i + 1)] > theData[(2*1 + 2)]) {
        					int temp = theData[2*i + 1];
        		    		theData[2*i + 1] = theData[i];
        		    		theData[i] = temp;
        		    		i = 2*i + 1;
        		    		
    					}	
    					else {
    						int temp = theData[2*i + 2];
        		    		theData[2*i + 2] = theData[i];
        		    		theData[i] = temp;
        		    		i = 2*i + 2;
    					}
    ...
    private void siftDown(int index) {
    	...
    	while (rightChild(index) < size) {
    		if (theData[rightChild(index)] > theData[index] || theData[leftChild(index)] > theData[index]) {
    			if (theData[rightChild(index)] > theData[leftChild(index)]){
    				int temp = theData[rightChild(index)];
    				theData[rightChild(index)] = theData[index];
					theData[index] = temp;
					index = rightChild(index);
    			}
    			else {
    				int temp = theData[leftChild(index)];
    				theData[leftChild(index)] = theData[index];
    				theData[index] = temp;
