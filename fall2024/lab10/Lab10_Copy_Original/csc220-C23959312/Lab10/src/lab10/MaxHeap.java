...
public class MaxHeap {
    ...
    public int poll() {
        ...
        if (size > 0) {
            ...
            while (!isLeaf(currentPos)) {
                int largerChildPos = leftChild(currentPos);
                
                if (rightChild(currentPos) < size && 
                    theData[rightChild(currentPos)] > theData[largerChildPos]) {
                    largerChildPos = rightChild(currentPos);
                }
                
                if (largerChildPos >= size || theData[currentPos] >= theData[largerChildPos]) {
                    break;
                }
                
                swap(currentPos, largerChildPos);
                currentPos = largerChildPos;
            }
        }
    ...
    private void siftDown(int index) {
        // TODO Assignment Part 1
    	 while (!isLeaf(index)) {
    	        int largestChild = leftChild(index);
    	        
    	        int rightChildIndex = rightChild(index);
    	        if (rightChildIndex < size && theData[rightChildIndex] > theData[largestChild]) {
    	            largestChild = rightChildIndex;
    	        }
    	        
    	        if (theData[index] >= theData[largestChild]) {
    	            break;
    	        }
