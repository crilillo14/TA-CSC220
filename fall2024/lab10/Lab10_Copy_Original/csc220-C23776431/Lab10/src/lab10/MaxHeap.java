...
public class MaxHeap {
    ...
    public boolean IsEqual(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++)
            if (arr[i] != theData[i]) {
            	System.out.println("shit " + arr[i] + " " + theData[i]);
                return false;
            }
    ...
    public int poll() {
    	...
    	while (!isLeaf(current)) {
    		if (theData[current] < theData[leftChild(current)] ||
    				theData[current] < theData[rightChild(current)]) {
    ...
    private void siftDown(int index) {
    	...
    	if (rightChild(index) >= size) {
	    	...
	    	if (theData[index] < theData[leftChild(index)] ||
					theData[index] < theData[rightChild(index)]) {
