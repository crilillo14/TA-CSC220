...
//I have tried to attend office hours for this lab and I still keep getting these error messages.
// I am not sure what the issue is, but I am submitting it because it's the deadline
...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	if (size >= theData.length) return;
    	
    	theData[size] = element; 
    	
    	int current = size;
    	
    	while (current > 0 && theData[current] > theData[parent(current)]) { // sifts up to maintain max-heap property
    ...
    public int poll() {
    	...
    	if (size == 0) return -1; //only if the heap is empty
     ...
     */
    public void sort(int[] arr) { // NOT SURE WHAT I AM MISSING HERE...
    ...
    private void siftDown(int index) {
        ...
        while (!isLeaf(index)) {
            ...
            if (largest == index) break; // only if the largest is still the index
