...
public class MaxHeap {
    ...
    public MaxHeap(int[] arr){
        ...
        // Use offer to add each element from the input array
        for (int element : arr) {
            offer(element);
        }
    }
    ...
    public int poll() {
        ...
        int max = theData[0]; // Store the max value
        theData[0] = theData[size - 1]; // Replace root with last leaf
        ...
        // Set indices greater than the new size to zero
        for (int i = size; i < theData.length; i++) {
            theData[i] = 0;
        }
    ...
    public void sort(int[] arr) {
        ...
        heapify(size - 1);  // Creates a max-heap from the entire array

        for (int i = size - 1; i > 0; i--) {
            swap(0, i);  // Move the root (max element) to the end of the heap
            size--;
            siftDown(0);  // Restore heap property in the reduced heap
        }
        ...
        size = arr.length;  // Reset size after sorting
    }
