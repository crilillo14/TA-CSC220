...
public class MaxHeap {
    ...
    public void offer(int element) {
        // TODO for Lab Part 3
    	// make sure to indent size, swap up 
    	if(size() == 0) {
    		theData[0] = element; 
    	} 
    	else {
    	...
    	int spot = size(); 
    	int parentSpot = parent(size()); 
    	
    	while(theData[spot] > theData[parentSpot]) {
    		swap(spot, parentSpot); 
    		spot = parentSpot; 
    		parentSpot = parent(spot); 
    	}
    ...
    public int poll() {
    	...
    	while(theData[leftChild(spot)] > theData[spot] || theData[rightChild(spot)] > theData[spot]) {
    		if (theData[leftChild(spot)] > theData[rightChild(spot)])
    			
    			swap(leftChild(spot),spot); 
    			spot = leftChild(spot); 
    			
    		}
    		if(theData[rightChild(spot)] > theData[leftChild(spot)]) {
    			
    			swap(rightChild(spot),spot); 
    			spot = rightChild(spot); 
    		}
