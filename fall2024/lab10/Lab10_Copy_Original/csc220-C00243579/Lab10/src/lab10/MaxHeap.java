...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	int index_pos = this.size;
    	this.theData[index_pos] = element;
    	while (this.theData[parent(index_pos)] < element)
    	{
    		swap(parent(index_pos), index_pos);  
    		index_pos = parent(index_pos);
    	}
    ...
    public int poll() {
    	...
    	int index_pos = 0;
    	int max_element = theData[index_pos]; 
    	
    //	theData[index_pos] = 0;
    //	swap(index_pos, size);  
    	theData[index_pos] = theData[size-1];
    	size = size - 1;
    	
    	while (theData[index_pos] < theData[leftChild(index_pos)] ||
    	       theData[index_pos] < theData[rightChild(index_pos)])
    	{
    		if ((theData[index_pos] < theData[leftChild(index_pos)]) &&
    	  	   (theData[rightChild(index_pos)] < theData[leftChild(index_pos)]))
    	 	{
    			if (theData[index_pos] != 0) {
    				swap(index_pos, leftChild(index_pos));  
    				index_pos = leftChild(index_pos);
    			}
    			else {theData[leftChild(index_pos)] = 0;}
    		}
    		else if (theData[index_pos] < theData[rightChild(index_pos)]) 
    		{
    			if (theData[index_pos] != 0) {
    				swap(index_pos, rightChild(index_pos));  
    				index_pos = rightChild(index_pos);
    			}
    			else {theData[rightChild(index_pos)] = 0;}
    		}
    		else {index_pos = index_pos + 1;}
    	} 
    	theData[size] = 0;
    	return max_element;
    }
    ...
    private void siftDown(int index) {
    	...
    	while (index < size) {	
    		if (rightChild(index) < size) {// have (two children) both left and right children
    		//	System.out.println("check 00 siftDown ** index = " + index + " size = " + size);
    			if (theData[rightChild(index)] > theData[leftChild(index)]) {
    				if (theData[rightChild(index)] > theData[index]) {
    					swap(rightChild(index), index);    			
    					index = rightChild(index);
    				//	System.out.println("** check 05 siftDown ** index = " + index + " size = " + size);
    				}
    				else {
    					return;
    				}
    				...
    				if (theData[leftChild(index)] > theData[index]) {
    		...
    		else if (leftChild(index) < size) { // have only one left child
    			if (theData[leftChild(index)] > theData[index]) {
					swap(leftChild(index), index);    			
					index = leftChild(index);
				//	System.out.println("** check 10 siftDown ** index = " + index + " size = " + size);
				}
    			else {
					return;
				}
