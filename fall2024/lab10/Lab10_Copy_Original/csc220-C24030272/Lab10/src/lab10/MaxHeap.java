...
public class MaxHeap {
    ...
    public void offer(int element) 
    {
    	...
    	else
    	{
	    	...
	    	theData[currentIndex]=element;
	    	size++;
	    	while(theData[currentIndex]>theData[parent(currentIndex)])
	    	{
    ...
    public int poll() 
    {
    	...
    	int removed = theData[0];
    	theData[0]=theData[size-1];
    	theData[size-1]=0;
    	int currentIndex=0;
    	size--;
    	while (!(theData[currentIndex]>theData[leftChild(currentIndex)] && theData[currentIndex]>theData[rightChild(currentIndex)]))
    	{
    		if (theData[leftChild(currentIndex)]>theData[rightChild(currentIndex)])
    		{
    ...
    private void siftDown(int index)
    {
    	...
    	if(rightChild(index)>=size && theData[leftChild(index)]<theData[index])
    	{
    		return;
    	}
    	if (theData[leftChild(index)]>theData[index] || theData[rightChild(index)]>theData[index])
    	{
