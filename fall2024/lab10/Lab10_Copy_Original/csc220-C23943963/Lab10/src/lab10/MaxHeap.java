...
public class MaxHeap {
    ...
    public void offer(int element) {
        ...
        while (currentIndex > 0 && !(element < theData[this.parent(currentIndex)])) {
            
            this.swap(currentIndex, this.parent(currentIndex));
            
         // Update currentIndex to the parent's index after a swap to continue maintaining the heap property
    ...
    public int poll() {
        ...
        while (leftChild(currentIndex) < size) {  
            ...
            if (left < size && theData[left] > theData[largestIndex]) {
                largestIndex = left;
            }
            if (right < size && theData[right] > theData[largestIndex]) {
                largestIndex = right;
            }

            // If the largest child is greater than the current node, swap
