...
public class MaxHeap {
    ...
    public void offer(int element) {
    	...
    	while(pointer > 0 && theData[pointer] > theData[parent(pointer)]) {
    ...
    public int poll() {
        // TODO for Lab Part 4
    	  if (size == 0) {
    	    ...
    	    while (!isLeaf(pointer)) {
    	        int left = leftChild(pointer);
    	        int right = rightChild(pointer);
    	        int largerChild;
    	        ...
    	        if (theData[pointer] >= theData[largerChild]) {
    	            break;
    	        }
