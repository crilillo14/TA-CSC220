...
public class MergeSort<E extends Comparable<E>> {
    ...
    public double O(int n) {
    	...
    	double bigO = n * Math.log(n);
        return bigO; // placeholder
    }
    ...
    public void fit(E[] array) {
    	...
    	long time = end - start; // convert to microseconds
    ...
    private void sort(E[] array1, E[] array2, int first, int last) {
        ...
        while (a <= middle && b <= last) {
        	...
        	if (array1[a].compareTo(array1[b]) <= 0) {
                array2[i++] = array1[a++];  // Copy from left half and increment a
            } else {
                array2[i++] = array1[b++];  // Copy from right half and increment b
            }
