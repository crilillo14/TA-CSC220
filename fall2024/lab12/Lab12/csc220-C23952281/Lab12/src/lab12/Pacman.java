	...
	*/
	...
	private void buildGraph() {
		...
		try {
            ...
            // Read the first line for maze dimensions
            String line = input.readLine();
            String[] dimensions = line.split(" ");
            height = Integer.parseInt(dimensions[0]);  // first integer -> rows #
            width = Integer.parseInt(dimensions[1]);   // second integer -> columns #

            // Initialize the maze array
            maze = new Node[height][width];

            // Read the maze data row by row
            for (int i = 0; i < height; i++) {
                line = input.readLine();           
            ...
            e.printStackTrace();  // Print any IOException
        }
    }
