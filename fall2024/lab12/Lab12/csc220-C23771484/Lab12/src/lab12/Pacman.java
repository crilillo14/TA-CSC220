	...
	*/
		...
		try {
			...
			for(int i = 0; i< height; i++) {
				for(int j = 0; j<width; j++) {
					output.println(maze[i][j].content + " ");
				} output.println();
			}
		} catch(IOException e) {
			e.printStackTrace();
	...
	public ArrayList<Node> getNeighbors(Node currentNode) {
		...
		if(inMaze(currentNode.row -1, height)
				&& inMaze(currentNode.col,width)
				&& ! maze[currentNode.row -1][currentNode.col].isWall()
		...
		if(inMaze(currentNode.row, height)
				&& inMaze(currentNode.col-1,width)
				&& ! maze[currentNode.row][currentNode.col-1].isWall()
	...
	public static void main(String[] args) {
		Pacman pacman = new Pacman("classic.txt", "classicBFSSol.txt");
		pacman.solveDFS();
