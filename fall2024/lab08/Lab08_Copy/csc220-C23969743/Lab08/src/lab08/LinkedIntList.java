...
public class LinkedIntList {
    ...
    public int lastIndexOf(int value) {
    	// Lab part 2.1
        int lastIndex = -1; // Initialize the lastIndex to -1 (not found)
        int currentIndex = 0; // Track the current index
        ListNode current = front; // Start at the front of the list
        
        // Traverse the list
        while (current != null) {
            if (current.data == value) {
                lastIndex = currentIndex; // Update lastIndex if value is found
            }
    ...
    public void removeAll(int value) {
        // TODO: Lab part 2.2
        // Handle the case where the front node contains the value
        while (front != null && front.data == value) {
            front = front.next; // Move the front to the next node
        }
        
        ListNode current = front; // Start traversal from the new front
    ...
    public void stutter() {
        ...
        while (current != null) {
            ...
            current = duplicate.next;  // Move to the node after the newly added duplicate
        }
    }
    ...
    public void shift() {
        ...
        if (front == null || front.next == null) {
            return;  // If the list is empty or has only one element, nothing to shift
        }

        ListNode evenHead = front;      // First node (index 0) is even
        ListNode oddHead = front.next;  // Second node (index 1) is odd

        ListNode even = evenHead;
        ListNode odd = oddHead;

        // Loop through the list, separating even and odd nodes
        ...
        // Attach the odd indexed nodes to the end of the even indexed nodes
        even.next = oddHead;
    }
