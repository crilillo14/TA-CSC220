...
public class LinkedIntList {
    ...
    public int lastIndexOf(int value) {
        ...
        while (current != null) {
            if (current.data == value) {
                lastIndex = currentIndex; // Update the last index if value matches
            }
            current = current.next;
            currentIndex++;
        }
    ...
    public void removeAll(int value) {
        // Handle the case where the front nodes contain the value
    ...
    public void stutter() {
        ...
        // Traverse the list
        while (current != null) {
            ListNode duplicate = new ListNode(current.data, current.next); // Create a duplicate node
            current.next = duplicate; // Insert the duplicate immediately after the current node
    ...
    public void shift() {
        if (front == null || front.next == null) {
            return; // If the list is empty or has only one element, no need to shift
        }

        ListNode evenHead = front; // Head of the even-indexed nodes
        ListNode oddHead = front.next; // Head of the odd-indexed nodes
        ListNode evenCurrent = evenHead;
        ListNode oddCurrent = oddHead;

        // Traverse and split the list into even and odd positions
        while (evenCurrent != null && oddCurrent != null && evenCurrent.next != null) {
            evenCurrent.next = oddCurrent.next; // Link even nodes together
            evenCurrent = evenCurrent.next; // Move to the next even node

            if (evenCurrent != null) {
                oddCurrent.next = evenCurrent.next; // Link odd nodes together
                oddCurrent = oddCurrent.next; // Move to the next odd node
            }
        }

        // Attach the odd-indexed nodes list to the end of the even-indexed nodes list
