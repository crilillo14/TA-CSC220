...
public class LinkedIntList {
    ...
    public void removeAll(int value) {
		...
		// we need to update front when removing the 1st element
    	...
    	while (curr != null) {
    		if (curr.data == value) {
    			...
    			prev = curr;
    			//move prev to curr position if no deletion occurs
    ...
    public void shift() {
    	...
    	if (front == null || front.next == null) {
    		return; // do nothing if list is empty or only has one node
    	}
    	ListNode evenOriginal = front;
    	ListNode oddNew = front.next;
    	ListNode oddBegin = oddNew;
    	
    	//traverse and group even and odd list
    	while (evenOriginal.next != null && oddNew.next != null) {
    		evenOriginal.next = evenOriginal.next.next;
    		evenOriginal = evenOriginal.next;
    		oddNew.next = oddNew.next.next;
    		oddNew = oddNew.next;
    	}
    	
    	evenOriginal.next = oddBegin; // take last thing from original list and make it point to first thing in new list
