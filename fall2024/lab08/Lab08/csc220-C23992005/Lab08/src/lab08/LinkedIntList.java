...
public class LinkedIntList 
{
    ...
    {
        // TODO: Lab part 2.1
    	return lastIndexOf(front,value,-1,0);
    }
    
    // Implementing recursion for traversing the list while keeping track of the last index
    private int lastIndexOf(ListNode node, int value, int lastFoundIndex, int currentIndex)
    {
    	if(node == null)
    	{
    	...
    	return lastIndexOf(node.next, value, lastFoundIndex, currentIndex + 1);
    }
    ...
    public int removeAll(int value) 
    {
    	...
    	// This aims to handle the corner case, where the front nodes contain such value
    	...
    	// Traversal throughout the entire existing list
    	ListNode current = front;
    ...
    private void stutterHelper(ListNode node)
    {
    	if(node == null)
    	{
    		return;
    	...
    	// Inserting the duplicate from the current node
    	ListNode duplicate = new ListNode(node.data);
    	duplicate.next = node.next;
    	node.next = duplicate;
    	
    	// Implementing recursion for calling the stutterHelper on the next original node
    	if(duplicate.next != null)
    	{
    		stutterHelper(duplicate.next);
    	}
    }
    ...
    public void shift() 
    {
    	...
    	shiftHelper(evenHead,oddHead);
    	
    }
    
    // Implementing a helper function to seperate odd and even indexed nodes
    private void shiftHelper(ListNode even, ListNode odd)
    {
    	...
    	// Processing and rearranging even and odd nodes
