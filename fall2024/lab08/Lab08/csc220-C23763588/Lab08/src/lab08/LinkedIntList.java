...
public class LinkedIntList {
    ...
    public void removeAll(int value) {
        // TODO: Lab part 2.2
    	// Handle cases where the front nodes need to be removed
        while (front != null && front.data == value) {
            front = front.next;
        }
        
        // If the list is now empty - done
        if (front == null) {
            return;
        }
        
        // Removes occurrences from the rest of the list
        ListNode current = front;
        while (current.next != null) {
    ...
    public void shift() {
    	...
    	if (front == null || front.next == null) {
            return; // list is empty or has only one element so nothing to shift
        }
