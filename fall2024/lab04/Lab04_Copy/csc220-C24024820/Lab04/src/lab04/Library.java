...
//   *********************************************************************
   ...
   public List<LibraryBook<Type>> getOrderedByAuthor() {
	   ArrayList<LibraryBook<Type>> a = new ArrayList<LibraryBook<Type>>();
	   a.addAll(library);

	   AuthorComparator c = new AuthorComparator();
   ...
   public List<LibraryBook<Type>> getOverdueList() {
	   ...
	   GregorianCalendar g = new GregorianCalendar(date.getYear(),date.getMonthValue()-1,date.getDayOfMonth());
	   ArrayList<LibraryBook<Type>> a = new ArrayList<LibraryBook<Type>>();
	   for(LibraryBook<Type> l:library) {
		   if(l.getDueDate()!=null) {
			   if(l.getDueDate().compareTo(g)>0) {
				   a.add(l);
			   }
		   }
	   }
	   DueDateComparator d = new DueDateComparator();
	   sort(a, d);
	   return a;
   }
