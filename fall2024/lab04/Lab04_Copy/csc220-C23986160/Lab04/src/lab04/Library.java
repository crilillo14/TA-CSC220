...
public class Library<Type> { // TODO: Lab Part 2 Make the Library class generic (don't forget to update the
	...
	/**
	 * Retrieves a list of library books sorted by the author's full name. If two
	 * books have the same author, the tie is broken by the book title.
	 *
	 * The sorting is performed using the AuthorComparator class, which compares the
	 * full author strings and then compares titles if the authors are the same.
	 *
	...
	public List<LibraryBook<Type>> getOrderedByAuthor() {
		...
		// i would like to write the selection sort
		// myself just for practicing purposes
		for (int i = 0; i < library.size() - 1; i++) {

			int minIndex = i;
			for (int j = i + 1; j < library.size(); j++) {

				if (authorComparator.compare(library.get(j), library.get(minIndex)) < 0) {
					minIndex = j;
				}

			}

			LibraryBook<Type> temp = library.get(i);
	...
	/**
	 * Retrieves a list of overdue library books sorted by the due date, with the
	 ...
	 *
	 * Be cautious of books with a null due date, as they may cause exceptions
	 * during comparison.
	 *
	...
	public List<LibraryBook<Type>> getOverdueList() {
		...
		// first i gotta filter out the overdue books
		...
		for (int i = 0; i < library.size(); i++) {
			...
			if (library.get(i).getDueDate() != null) {
				...
				if ( today.compareTo(library.get(i).getDueDate()) > 0 ) {
					overDueBooks.add(library.get(i));
				}
			}
		}


		sort(overDueBooks, dueDateComparator);
	...
	protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
		...
		// RETURN RULES
		// returns -1 if book1's author is before book2's author alphabetically
		// returns 0 if they are the same author and title
		// returns 1 if book1's author is after book2's author alphabetically
	...
	protected class DueDateComparator implements Comparator<LibraryBook<Type>> {
		...
		// RETURN RULES
		// returns -1 if book1 is due before book2 OR if book2 is null
		// returns 0 if they are due at the same time OR if both are null
		// returns 1 if book1 is due after book2 OR if book1 is null

		public int compare(LibraryBook<Type> book1, LibraryBook<Type> book2) {
			GregorianCalendar dueDate1 = book1.getDueDate();
			GregorianCalendar dueDate2 = book2.getDueDate();

			// need to make sure the duedates aren't null
