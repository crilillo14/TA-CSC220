...
public class LibraryGenericTest {
  ...
  private static void testGetInventoryList() {
    ...
    if (sortedBooks == null || sortedBooks.size() != 3) {
      System.err.println("TEST FAILED: getInventoryList - List is null or incorrect size");
      return;
    }

    // Expected order of ISBNs: 9780330351690, 9780374292799, 9780446580342
    long[] expectedIsbns = {
      9780330351690L, // Jon Krakauer
      9780374292799L, // Thomas L. Friedman
      9780446580342L  // David Baldacci
    };

    
    for (int i = 0; i < sortedBooks.size(); i++) {
      if (sortedBooks.get(i).getIsbn() != expectedIsbns[i]) {
        System.err.println("TEST FAILED: getInventoryList - Books are not sorted by ISBN");
        return;
      }
    }

    System.out.println("getInventoryList test passed.");
  }
}
