...
public class Library<Type> { //TODO: Lab Part 2 Make the Library class generic (don't forget to update the holder)
  ...
  //  */
   protected class IsbnComparator implements Comparator<LibraryBook<Type>> {
	...
	public int compare(LibraryBook<Type> book1, LibraryBook<Type> book2) {
		...
		if (isbn1 - isbn2 < 0) {
			return (int)(isbn1 - isbn2);
		} else if (isbn1 - isbn2 > 0) {
			return (int)(isbn1 - isbn2);
		} 
		// if neither of these, then 0 (they are equal)
		return 0;
	}
  ...
  //  */
   public List<LibraryBook<Type>> getOverdueList() {
	   ...
	   GregorianCalendar todayDate = new GregorianCalendar(); // should give today's date
	   for (LibraryBook<Type> book : library) {
		   GregorianCalendar dueDate = book.getDueDate();
		   if (dueDate != null && dueDate.compareTo(todayDate) < 0){
  ...
  //  */
   protected class DueDateComparator implements Comparator<LibraryBook<Type>> {
	   public int compare(LibraryBook<Type> book1, LibraryBook<Type> book2) {
		   ...
		   //specific cases to deal with potential null due date books
		   if(dueDate1 == null && dueDate2 == null) {
			   return 0;
		   } else if (dueDate1 == null) {
			   return 1; // book 1 has no due date so it will come after book2
