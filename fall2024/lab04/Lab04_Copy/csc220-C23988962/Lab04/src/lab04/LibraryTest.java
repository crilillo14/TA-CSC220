...
public class LibraryTest {
  ...
  public static void main(String[] args) {
    ...
    // Test lookup by ISBN for a book in the medium library
    if (lib.lookup(9781843190004L) != null) {
        System.err.println("TEST FAILED -- medium library: lookup(isbn)");
    }

    // Test checking out a book from the medium library
    if (!lib.checkout(9781843190004L, "Alice", 12, 12, 2024)) {
        System.err.println("TEST FAILED -- medium library: checkout");
    }

    // Verify the book has been checked out by "Alice"
    booksCheckedOut = lib.lookup("Alice");
    if (booksCheckedOut == null || booksCheckedOut.size() != 1
        || !booksCheckedOut.get(0).equals(
            new Book(9781843190004L, "Moyra Caldecott", "Weapons of the Wolfhound"))) {
        System.err.println("TEST FAILED -- medium library: lookup(holder)");
    }

    // Test checking in the book
    if (!lib.checkin(9781843190004L)) {
        System.err.println("TEST FAILED -- medium library: checkin(isbn)");
    }

    // Verify the book has been checked in (should return null for the holder)
    if (lib.lookup(9781843190004L) != null) {
        System.err.println("TEST FAILED -- medium library: lookup after checkin");
    }

    // Test sorting the library by ISBN
    ArrayList<LibraryBook<String>> sortedByISBN = lib.getInventoryList();
    if (sortedByISBN == null || sortedByISBN.isEmpty()) {
