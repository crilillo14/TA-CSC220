...
public class Library<Type> { //TODO: Lab Part 2 Make the Library class generic (don't forget to update the holder)
  ...
  // }
	public List<LibraryBook<Type>> getOverdueList() {
		  ...
		  GregorianCalendar today = new GregorianCalendar(); // Get today's date
		  
		  for (LibraryBook<Type> book : library) {
		    if (book.getDueDate() != null && book.getDueDate().before(today)) {
		      // Only add books with a due date before today (overdue)
		      overdueBooks.add(book);
		    }
		  }
		  
		  
		  sort(overdueBooks, new DueDateComparator());
  ...
  // }
	protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
		  ...
		  public int compare(LibraryBook<Type> book1, LibraryBook<Type> book2) {
		    ...
		    int compareNames = book1.getAuthor().toLowerCase().compareTo(book2.getAuthor().toLowerCase());
		    if (compareNames != 0) {
		      return compareNames;
		    }
