...
public class Library<Type> { // TODO: Lab Part 2 Make the Library class generic (don't forget to update the
	...
	protected class IsbnComparator implements Comparator<LibraryBook<Type>> {
		// compare o1 and o2 -> positive difference means o1 is bigger
		// negative difference -> o2 is bigger
		// difference = 0 -> o1 = o2
	...
	// *
	// * This method filters the library books to only include those that are
	// overdue,
	...
	public ArrayList<LibraryBook<Type>> getOverdueList() {
		...
		for (int i = 0; i < library.size(); i++) {
			if (library.get(i).getDueDate() != null) {
				if (library.get(i).getDueDate().before(new GregorianCalendar())) {
	...
	protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
		...
		public int compare(LibraryBook<Type> b1, LibraryBook<Type> b2) {
			if (b1.getAuthor().toUpperCase().compareTo(b2.getAuthor().toUpperCase()) > 0) {
				return 1;
			} else if (b1.getAuthor().compareTo(b2.getAuthor()) < 0) {
				return -1;
			}

			if (b1.getTitle().toUpperCase().compareTo(b2.getTitle().toUpperCase()) > 0) {
				return 1;
