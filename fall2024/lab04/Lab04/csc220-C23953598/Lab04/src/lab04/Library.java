...
public class Library<Type> {
  ...
  protected class IsbnComparator implements Comparator<LibraryBook<Type>> {
      ...
      public int compare(LibraryBook<Type> lhs, LibraryBook<Type> rhs) {
    	  if (lhs.getIsbn() < rhs.getIsbn()) {
    		  return -1;
    	  } else if (lhs.getIsbn() > rhs.getIsbn()) {
    		  return 1;
    	  } else {
    		  return 0;
    	  }
  ...
  public ArrayList<LibraryBook<Type>> getOverdueList(int month, int day, int year) {
	  ...
	  for (int i = 0; i < library.size(); i++) {
		  if (library.get(i).getDueDate() != null) {
			  if (temp.compareTo(library.get(i).getDueDate()) > 0) {
 ...
 /**
  ...
  protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
	  ...
	  public int compare(LibraryBook<Type> lhs, LibraryBook<Type> rhs) {
		  String lhsComb = lhs.getAuthor() + lhs.getTitle();
		  String rhsComb = rhs.getAuthor() + rhs.getTitle();
		  
		  if (lhsComb.compareTo(rhsComb) < 0) {
			  return -1;
		  } else if (lhsComb.compareTo(rhsComb) > 0){
			  return 1;
		  } else {
			  return 0;
		  }
  ...
  protected class DueDateComparator implements Comparator<LibraryBook<Type>> {
	  ...
	  public int compare(LibraryBook<Type> lhs, LibraryBook<Type> rhs) {
		  GregorianCalendar lhsD = lhs.getDueDate();
		  GregorianCalendar rhsD = rhs.getDueDate();
