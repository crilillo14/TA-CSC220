...
public class LibraryGenericTest {
  ...
  public static void main(String[] args) {
    ...
    for (LibraryBook book : lib3.getOrderedByAuthor()) {
    	System.out.println(book.getIsbn() + " --- "+ book.getAuthor() + " --- " + book.getTitle());
    }
    LibraryBook testBook = null;
    for (LibraryBook book : lib3.getOrderedByAuthor()) {
        if (book.getIsbn() == 9781843193319L) {
            testBook = book;
            break; // Exit the loop once the book is found
        }
    }
    if (lib3.checkout(9781843193319L, "patron2", 1, 1, 2008))
        System.out.println(testBook.getDueDate());
    
    int j = lib3.getOrderedByAuthor().size();
    for (LibraryBook book : lib3.getOrderedByAuthor()) {
    	lib3.checkout(book.getIsbn(), "patron2", j, 1, 2008);
    	j--;
    }
    
    for (LibraryBook book : lib3.getOverdueList()) {
    	System.out.println(book.getIsbn() + " --- "+ book.getAuthor() + " --- " + book.getTitle());
    }
