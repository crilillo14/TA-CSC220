...
assignment = "Lab02"
assignmentfiles = ["Matrix.java"]
disk_main_add = "/Users/Jerry/Documents/TA/fall2017/"
compile_files =  ["Matrix.java", "MatrixRef.java"]
main_file = "CheckLab.java"
main_class = "CheckLab"
actual_point = [15,5,15,10,15,15,15,10]
...
def init_comment_file(filename,comments):
	"""
	initialize comments for the student  
	"""
...
def assignment_checking_rubric_all(filename,student = None,points=None, is_late = False):
	...
	if  points is None:
		file = open(filename,"w");
		file.write("Last Name, First Name, Lab id, Submitted, Correct Submitted, Compile, Run, Equal, Plus, Times, Transpose, Late, Total")
	else:
		file = open(filename,"a+");
...
def check_assignment_for_student(dist_disk):
	...
	submission_wrong = []
	compiler_error = []
	submission_status = ["Submitted","Correct Submitted", "Compile",
	"Run","Equal","Plus","Times", "Transpose"]
	...
	for student in students:
		...
		submission_point = [0,0,0,0,0, 0, 0, 0]
		...
		no_submission = False
		init_comment_file(stu_lab_comment, student[0] + "\t\t" + student[1] +  " " + student[2] + "\n")
		...
		if is_copy:
			...
			# copy MatrixRef.java into each of the student's lab folder 
			...
			if is_compiled is None and is_run_successfully is None:
				...
				if(int(message[0]) == 1):
					submission_point[-4] = 10;
				else:
					assignment_checking_comments(stu_lab_comment," ## equal() is incorrect\n")

				if(int(message[1]) == 1) and (int(message[0]) == 1):
					submission_point[-4] = 15;
				else:
					assignment_checking_comments(stu_lab_comment," ## equal() does not check all cases\n")

				if(int(message[2]) == 1):
					submission_point[-3] = 10;
				else:
					assignment_checking_comments(stu_lab_comment," ## plus() is incorrect \n")

				if(int(message[3]) == 1) and (int(message[2]) == 1):
					submission_point[-3] = 15;
				else:
					assignment_checking_comments(stu_lab_comment," ## plus() does not check all cases\n")

				if(int(message[4]) == 1):
					submission_point[-2] = 10;
				else:
					assignment_checking_comments(stu_lab_comment," ## times() is incorrect \n")

				if(int(message[5]) == 1) and (int(message[4]) == 1):
					submission_point[-2] = 15;
				else:
					assignment_checking_comments(stu_lab_comment," ## times() does not check all cases\n")
				
				if(int(message[6]) == 1):
					submission_point[-1] = 5;
				else:
					assignment_checking_comments(stu_lab_comment," ## transpose() is incorrect \n")

				if(int(message[6]) == 1) and (int(message[7]) == 1):
					submission_point[-1] = 10;
				else:
					assignment_checking_comments(stu_lab_comment," ## transpose() does not check all cases \n")


		elif no_submission is False:
			assignment_checking_comments(stu_lab_comment,"Your submission is wrong. You may not have submitted all required files.\n")
			assignment_checking_comments(stu_lab_comment, "Does your folder have? " + compile_files[0] + "\n")
