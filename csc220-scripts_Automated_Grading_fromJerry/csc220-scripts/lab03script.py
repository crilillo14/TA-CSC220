...
assignment = "Lab03"
assignmentfiles = ["Book.java", "Library.java", "LibraryBook.java", "LibraryTest.java"]
disk_main_add = "/Users/Jerry/Documents/TA/fall2017/"
compile_files =  ["Book.java", "Library.java", "LibraryBook.java"]
main_file = "CheckLab.java"
main_class = "CheckLab"
actual_point = [15,5,15,10,15,20,20]


def check_correct_assignment_submission(distadd,assignment,filelist):
	"""
	if the student does not have all files listed in the assignmentfiles list in
	the Box folder, then this is marked as an incomplete submission. 
	"""
...
def assignment_checking_rubric_all(filename,student = None,points=None, is_late = False):
	...
	if  points is None:
		file = open(filename,"w");
		file.write("Last Name, First Name, Lab id, Submitted, Correct Submitted, Compile, Run, checkin(isbn), checkin(holder), lookup(holder), Late, Total")
	else:
		file = open(filename,"a+");
...
def check_assignment_for_student(dist_disk):
	...
	submission_wrong = []
	compiler_error = []
	submission_status = ["Submitted","Correctly Submitted", "Compiles",
	"Runs","checkin(isbn)","checkin(holder)","lookup(holder)"]
	...
	for student in students:
		...
		submission_point = [0,0,0,0,0,0,0]
		...
		if is_copy:
			...
			if is_compiled is None and is_run_successfully is None:
				...
				submission_point[4] = int(message[0]);
				submission_point[5] = int(message[1]);
				submission_point[6] = int(message[2]);

				assignment_checking_comments(stu_lab_comment,comments+"\n")

		elif no_submission is False:
			assignment_checking_comments(stu_lab_comment,"Your submission is incomplete. You may not have submitted all required files.\n")
			assignment_checking_comments(stu_lab_comment, "Does your folder have? " + assignmentfiles[0] + " " \
				+ assignmentfiles[1] + " " + assignmentfiles[2] + " " + assignmentfiles[3] + "\n")
