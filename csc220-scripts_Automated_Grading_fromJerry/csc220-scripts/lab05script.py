...
assignment = "Lab05"
assignmentfiles = ["BinarySearchSet.java","Tester.java"]
disk_main_add = "/Users/Jerry/Documents/TA/fall2017/"
compile_files =  ["BinarySearchSet.java","Tester.java"]
...
# remove() -> 10 points
# contains() -> 10 points
# containsAll() -> 5 points
# binary_add()/sequential_add() -> 8 points 
# BinarySearchSet([]) -> 7 points 
actual_point = [15,5,10,5,5,5,5,5,5,10,10,5,8,7]
...
def check_shared_folder(distadd,assignment,assignmentfiles):
	...
	file = open(disk_main_add+"housekeeping-"+assignment.lower()+".txt","w")
...
def assignment_checking_rubric_all(filename,student = None,points=None, is_late = False):
	...
	if  points is None:
		...
		file.write("Last Name, First Name, Lab ID, Submitted, Correct Submission, " +
			"Compiles, Runs, BinarySearchSet(), isEmpty(), size(), clear()," +  
			"grow(), remove(), contains(), containsAll(), *_add(), BinarySearchSet([]), Late, Total")
	else:
		file = open(filename,"a+");
...
def check_assignment_for_student(dist_disk):
	...
	"Runs","BinarySearchSet()","isEmpty()", "size()", "clear()", "grow()", "remove()", 
	"contains()", "containsAll()", "*_add()", "BinarySearchSet([])"]
	...
	for student in students:
		...
		if is_copy:
			...
			assignment_checking_comments(stu_lab_comment, "Does your folder have? " + compile_files[0] + " " + compile_files[1] + "\n")
